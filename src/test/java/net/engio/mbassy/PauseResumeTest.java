...
public class PauseResumeTest extends MessageBusTest {

    private SuspendableMBassador bus;
    ...
    public void testSyncWithSyncHandlersUnpausedPublication ()
    {
        ...
        bus = createSuspendableBus (SyncAsync (), listeners);
	...
	final Runnable publishAndCheck = new Runnable() {
		...
		final SyncAsyncPausePostCommand stdmsg = bus.post(standardMessage);
		final SyncAsyncPausePostCommand mpmsg = bus.post(multipartMessage);
		final SyncAsyncPausePostCommand smsg = bus.post(MessageTypes.Simple);
    ...
    private SuspendableMBassador createSuspendableBus(final IBusConfiguration configuration) {
        return new SuspendableMBassador (configuration);
    }

    private SuspendableMBassador createSuspendableBus(final IBusConfiguration configuration, final ListenerFactory listeners) {
        final SuspendableMBassador bus = new SuspendableMBassador (configuration);
        ConcurrentExecutor.runConcurrent(TestUtil.subscriber(bus, listeners));
        return bus;
    }
}
